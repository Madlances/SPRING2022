export default class ListPageView {
  ...
  async renderWrapper() {
    ...
    if (!this.wrapperTemplateHtml.length > 0) {
      this.wrapperTemplateHtml = await this.getFileContents(
        this.wrapperTemplateUrl
      );
    }
    this.$wrapperContainer.html(
      ejs.render(this.wrapperTemplateHtml, { view: this.viewModel })
    );
  ...
  async renderList() {
    ...
    this.$listContainer.html(
      ejs.render(this.listTemplateHtml, { view: this, data: this.data })
    );
